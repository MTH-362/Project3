...
```{r, echo = FALSE}
coffee.new[,c('species',
              'owner','country_of_origin','farm_name','lot_number','mill','ico_number','company',
              'altitude','producer','bag_weight','in_country_partner','harvest_year','grading_date',
              'owner_1','variety','certification_body','certification_address','expiration',
              'certification_contact','unit_of_measurement','region',
              'altitude_low_meters','altitude_high_meters','altitude_mean_meters')] <- list(NULL)
glimpse(coffee.new)
...
```{r}
naCoffee = coffee_ratings2 %>% drop_na()
```

```{r, trace = 0}
stepwise <- lm(total_cup_points ~ ., naCoffee)
